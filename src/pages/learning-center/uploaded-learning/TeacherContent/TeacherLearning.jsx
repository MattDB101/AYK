...
function TeacherContentLearning() {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const { learningContent, loading, error } = useTeacherLearningContent();

  const groupedLearningContent = {
    safety: learningContent.filter((item) => item.category === 'safety'),
    techniques: learningContent.filter((item) => item.category === 'techniques'),
    management: learningContent.filter((item) => item.category === 'management'),
  };

  const handleRecipeClick = (contentId) => {
    navigate(`/learning/teacher-content/${contentId}`);
  };
...
export default TeacherContentLearning;
